...
<details>
    <summary>Отличия режимов сканирования</summary>
    <pre>
**SYN-сканирование (-sS):**
1. **Протокол:** TCP.
2. **Отправляемые флаги Nmap:** Отправляется только флаг SYN.
3. **Дополнение:** Это "полуоткрытое" сканирование, так как оно не завершает полное TCP-рукопожатие.

**FIN-сканирование (-sF):**
1. **Протокол:** TCP.
2. **Отправляемые флаги Nmap:** Отправляется только флаг FIN.
3. **Дополнение:** Часто используется для обхода простых stateful файерволов, которые могут отслеживать SYN-пакеты, но пропускают FIN-пакеты, если нет установленного соединения.

**Xmas-сканирование (-sX):**
1. **Протокол:** TCP.
2. **Отправляемые флаги Nmap:** Отправляются флаги FIN, PSH и URG.
3. **Дополнение:** Как и FIN-сканирование, может использоваться для обхода файерволов.

**UDP-сканирование (-sU):**
1. **Протокол:** UDP.
2. **Отправляемые пакеты Nmap:** Отправляются UDP-пакеты на целевые порты. Эти пакеты могут быть пустыми или содержать специфические данные для определенных служб (например, DNS-запрос, если Nmap пытается определить DNS-сервер).
3. **Дополнение:** Поскольку UDP не имеет механизма установления соединения, Nmap не может полагаться на рукопожатия для определения состояния порта.
   </pre>
</details>
...
<details>
    ...
    <pre>
**SYN-сканирование (-sS):**
1. **Порт открыт:** Сервер отвечает пакетом SYN/ACK. Nmap в ответ отправляет RST для закрытия соединения, не завершая рукопожатие.
2. **Порт закрыт:** Сервер отвечает пакетом RST/ACK.

**FIN-сканирование (-sF):**
1. **Порт открыт:** Сервер не отправляет никакого ответа. Это связано с тем, что, согласно RFC 793 (TCP), если FIN-пакет получен на открытый порт без установленного соединения, он должен быть проигнорирован.
2. **Порт закрыт:** Сервер отправляет пакет RST.

**Xmas-сканирование (-sX):**
1. **Порт открыт:** Сервер не отправляет никакого ответа. (Поведение схоже с FIN-сканированием).
2. **Порт закрыт:** Сервер отправляет пакет RST.

**UDP-сканирование (-sU):**
1. **Порт открыт:** Сервер может ответить UDP-пакетом, указывающим на работающую службу (например, DNS-ответ, DHCP-ответ), либо не ответить вообще (для многих служб это означает "открыт/отфильтрован", и Nmap будет ждать таймаута).
2. **Порт закрыт:** Сервер отвечает ICMP-сообщением "Port Unreachable" (тип 3, код 3). Это является ключевым индикатором закрытого UDP-порта.
    </pre>
</details>

**Ссылка на файлы wireshark:**

[Файлы wireshark](nmap/wireshark/)
   
**Ссылка на fin_scan.txt:**

[Ссылка на fin_scan.txt](nmap/fin_scan.txt)
   
**Ссылка на syn_scan.txt:**

[Ссылка на syn_scan.txt](nmap/syn_scan.txt)
   
**Ссылка на xmas_scan.txt:**

[Ссылка на xmas_scan.txt](nmap/xmas_scan.txt)
   
**Ссылка на udp_scan.txt:**

[Ссылка на udp_scan.txt](nmap/udp_scan.txt)
