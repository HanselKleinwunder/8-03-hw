...
<pre>
1. **SYN-сканирование (-sS):**<br>
* Порт открыт: Сервер отвечает пакетом SYN/ACK. Nmap в ответ отправляет RST для закрытия соединения, не завершая рукопожатие.<br>
* Порт закрыт: Сервер отвечает пакетом RST/ACK.
2. **FIN-сканирование (-sF):**<br>
* Порт открыт: Сервер не отправляет никакого ответа. Это связано с тем, что, согласно RFC 793 (TCP), если FIN-пакет получен на открытый порт без установленного соединения, он должен быть проигнорирован.<br>
* Порт закрыт: Сервер отправляет пакет RST.
3. **Xmas-сканирование (-sX):**<br>
* Порт открыт: Сервер не отправляет никакого ответа. (Поведение схоже с FIN-сканированием).<br>
* Порт закрыт: Сервер отправляет пакет RST.
4. **UDP-сканирование (-sU):**<br>
* Порт открыт: Сервер может ответить UDP-пакетом, указывающим на работающую службу (например, DNS-ответ, DHCP-ответ), либо не ответить вообще (для многих служб это означает "открыт/отфильтрован", и Nmap будет ждать таймаута).<br>
