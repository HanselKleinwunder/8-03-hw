...
2. Как отвечает сервер?
   * SYN-сканирование (SYN scan): Если хост отвечает пакетом с флагами SYN-ACK, это указывает на то, что порт открыт. Этот метод сканирования известен как "трехстороннее рукопожатие" (three-way handshake).
   * FIN-сканирование (FIN scan): Этот метод используется для обхода файерволов. Если хост отправляет в ответ пакет с флагом RST (Reset), это означает, что порт закрыт. В случае, если ответа нет, порт считается открытым.
   * Xmas-сканирование (Xmas scan): При этом сканировании отправляются пакеты с флагами FIN, PSH и URG. Как и в случае с FIN-сканированием, если хост отвечает пакетом RST, порт считается закрытым. Отсутствие ответа, наоборот, указывает на открытый порт.
