...
**SYN-сканирование (-sS):**
**Протокол:** TCP.
**Отправляемые флаги Nmap:** Отправляется только флаг SYN.
**Дополнение:** Это "полуоткрытое" сканирование, так как оно не завершает полное TCP-рукопожатие.

**FIN-сканирование (-sF):**
**Протокол:** TCP.
**Отправляемые флаги Nmap:** Отправляется только флаг FIN.
...
**Протокол:** TCP.
**Отправляемые флаги Nmap:** Отправляются флаги FIN, PSH и URG.
**Дополнение:** Как и FIN-сканирование, может использоваться для обхода файерволов.

**UDP-сканирование (-sU):**
**Протокол:** UDP.
**Отправляемые пакеты Nmap:** Отправляются UDP-пакеты на целевые порты. Эти пакеты могут быть пустыми или содержать специфические данные для определенных служб (например, DNS-запрос, если Nmap пытается определить DNS-сервер).
...
**SYN-сканирование (-sS):**
**Порт открыт:** Сервер отвечает пакетом SYN/ACK. Nmap в ответ отправляет RST для закрытия соединения, не завершая рукопожатие.
**Порт закрыт:** Сервер отвечает пакетом RST/ACK.

**FIN-сканирование (-sF):**
**Порт открыт:** Сервер не отправляет никакого ответа. Это связано с тем, что, согласно RFC 793 (TCP), если FIN-пакет получен на открытый порт без установленного соединения, он должен быть проигнорирован.
**Порт закрыт:** Сервер отправляет пакет RST.

**Xmas-сканирование (-sX):**
**Порт открыт:** Сервер не отправляет никакого ответа. (Поведение схоже с FIN-сканированием).
**Порт закрыт:** Сервер отправляет пакет RST.

**UDP-сканирование (-sU):**
**Порт открыт:** Сервер может ответить UDP-пакетом, указывающим на работающую службу (например, DNS-ответ, DHCP-ответ), либо не ответить вообще (для многих служб это означает "открыт/отфильтрован", и Nmap будет ждать таймаута).
**Порт закрыт:** Сервер отвечает ICMP-сообщением "Port Unreachable" (тип 3, код 3). Это является ключевым индикатором закрытого UDP-порта.
</pre>
</details>
