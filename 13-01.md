...
<details>
  <summary>Цели задания</summary>
1. Скачайте и установите виртуальную машину Metasploitable: https://sourceforge.net/projects/metasploitable/. Это типовая ОС для экспериментов в области информационной безопасности, с которой следует начать при анализе уязвимостей.<br>
2. Просканируйте эту виртуальную машину, используя nmap.<br>
3. Попробуйте найти уязвимости, которым подвержена эта виртуальная машина. Сами уязвимости можно поискать на сайте https://www.exploit-db.com/. Для этого нужно в поиске ввести название сетевой службы, обнаруженной на атакуемой машине, и выбрать подходящие по версии уязвимости.<br>
4. Ответьте на следующие вопросы:
<ul>
    <li>Какие сетевые службы в ней разрешены?</li>
    <li>Какие уязвимости были вами обнаружены? (список со ссылками: достаточно трёх уязвимостей)</li>
    <li>Приведите ответ в свободной форме.</li> 
</ul>
</details>
...
<details>
  ...
  <pre>
  Starting Nmap 7.95 ( https://nmap.org ) at 2025-07-23 22:04 EDT
...
|   100005  1,2,3      38791/tcp   mountd
|   100005  1,2,3      43983/udp   mountd
|   100021  1,3,4      36198/udp   nlockmgr
|   100021  1,3,4      46451/tcp   nlockmgr
|   100024  1          37931/udp   status
|_  100024  1          59909/tcp   status
...
MAC Address: 00:0C:29:BD:35:F1 (VMware)
...
1. Проведите сканирование Metasploitable в режимах SYN, FIN, Xmas, UDP.
2. Запишите сеансы сканирования в Wireshark.
3. Ответьте на следующие вопросы:
   * Чем отличаются эти режимы сканирования с точки зрения сетевого трафика?
   * Как отвечает сервер?
4. Приведите ответ в свободной форме.

## Процесс выполнения:

### Чем отличаются эти режимы сканирования с точки зрения сетевого трафика?

Используемыми сетевыми протоколами и флагами пакетов:

<details>
  <summary>Режимы сканирования</summary>
<h4>1. SYN-сканирование (-sS):</h4>
<ul>
    <li><b>Протокол:</b> TCP.</li>
    <li><b>Отправляемые флаги Nmap:</b> Отправляется только флаг SYN.</li>
    <li><b>Дополнение:</b> Это "полуоткрытое" сканирование, так как оно не завершает полное TCP-рукопожатие.</li>
</ul>
<h4>2. FIN-сканирование (-sF):</h4>
<ul>
    <li><b>Протокол:</b> TCP.</li>
    <li><b>Отправляемые флаги Nmap:</b> Отправляется только флаг FIN.</li>
    <li><b>Дополнение:</b> Часто используется для обхода простых stateful файерволов, которые могут отслеживать SYN-пакеты, но пропускают FIN-пакеты, если нет установленного соединения.</li>
</ul>
<h4>3. Xmas-сканирование (-sX):</h4>
<ul>
    <li><b>Протокол:</b> TCP.</li>
    <li><b>Отправляемые флаги Nmap:</b> Отправляются флаги FIN, PSH и URG.</li>
    <li><b>Дополнение:</b> Как и FIN-сканирование, может использоваться для обхода файерволов.</li>
</ul>
<h4>4. UDP-сканирование (-sU):</h4>
<ul>
    <li><b>Протокол:</b> UDP.</li>
    <li><b>Отправляемые пакеты Nmap:</b> Отправляются UDP-пакеты на целевые порты. Эти пакеты могут быть пустыми или содержать специфические данные (например, DNS-запрос, если Nmap пытается определить DNS-сервер).</li>
    <li><b>Дополнение:</b> Поскольку UDP не имеет механизма установления соединения, Nmap не может полагаться на рукопожатия для определения состояния порта.</li>
</ul>
</details>
...
<details>
  <summary>Ответы серверов</summary>
<h4>1. SYN-сканирование (-sS):</h4>
<ul>
    <li><b>Порт открыт:</b> Сервер отвечает пакетом SYN/ACK. Nmap в ответ отправляет RST для закрытия соединения, не завершая рукопожатие.</li>
    <li><b>Порт закрыт:</b> Сервер отвечает пакетом RST/ACK.</li>
</ul>
<h4>2. FIN-сканирование (-sF):</h4>
<ul>
    <li><b>Порт открыт:</b> Сервер не отправляет никакого ответа. Это связано с тем, что, согласно RFC 793 (TCP), если FIN-пакет получен на открытый порт без установленного соединения, он должен быть проигнорирован.</li>
    <li><b>Порт закрыт:</b> Сервер отправляет пакет RST.</li>
</ul>
<h4>3. Xmas-сканирование (-sX):</h4>
<ul>
    <li><b>Порт открыт:</b> Сервер не отправляет никакого ответа. (Поведение схоже с FIN-сканированием).</li>
    <li><b>Порт закрыт:</b> Сервер отправляет пакет RST.</li>
</ul>
<h4>4. UDP-сканирование (-sU):</h4>
<ul>
    <li><b>Порт открыт:</b> Сервер может ответить UDP-пакетом, указывающим на работающую службу (например, DNS-ответ, DHCP-ответ), либо не ответить вообще (для многих служб это означает "открыт/отфильтрован", и Nmap будет ждать таймаута).</li>
    <li><b>Порт закрыт:</b> Сервер отвечает ICMP-сообщением "Port Unreachable" (тип 3, код 3). Это является ключевым индикатором закрытого UDP-порта.</li>
</ul>
</details>
...
1. [Файлы wireshark](wireshark/)

2. [Файл FIN-сканирования](nmap/fin_scan.txt)

3. [Файл SYN-сканирования](nmap/syn_scan.txt)

4. [Файл XMAS-сканирования](nmap/xmas_scan.txt)

5. [Файл UDP-сканирования](nmap/udp_scan.txt)
