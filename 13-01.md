...
1. **SYN-сканирование (-sS):**<br>
* Протокол: TCP.<br>
* Отправляемые флаги Nmap: Отправляется только флаг SYN.<br>
* Дополнение: Это "полуоткрытое" сканирование, так как оно не завершает полное TCP-рукопожатие.
2. **FIN-сканирование (-sF):**<br>
* Протокол: TCP.<br>
* Отправляемые флаги Nmap: Отправляется только флаг FIN.<br>
* Дополнение: Часто используется для обхода простых stateful файерволов, которые могут отслеживать SYN-пакеты, но пропускают FIN-пакеты, если нет установленного соединения.
3. **Xmas-сканирование (-sX):**<br>
* Протокол: TCP.<br>
* Отправляемые флаги Nmap: Отправляются флаги FIN, PSH и URG.<br>
* Дополнение: Как и FIN-сканирование, может использоваться для обхода файерволов.
4. **UDP-сканирование (-sU):**<br>
* Протокол: UDP.<br>
* Отправляемые пакеты Nmap: Отправляются UDP-пакеты на целевые порты. Эти пакеты могут быть пустыми или содержать специфические данные для определенных служб (например, DNS-запрос, если Nmap пытается определить DNS-сервер).<br>
* Дополнение: Поскольку UDP не имеет механизма установления соединения, Nmap не может полагаться на рукопожатия для определения состояния порта.
